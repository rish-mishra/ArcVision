"""
READ-ONLY forensic script for arcvision_demo_final.mov (session 29c7e751775e4f20).
Reproduces the exact same detection/tracking/state-machine pass production used,
plus keeps raw per-frame RF-DETR ball candidates (before tracker association) and
per-window trigger type, so a discarded-by-the-tracker detection can be told apart
from RF-DETR never finding anything, and a fallback-triggered window can be told
apart from a primary (ball-hand-proximity) one. Also directly re-derives, using the
REAL unmodified app.events.outcome_detector internals, exactly which evidence
branch produced (or failed to produce) each shot's outcome, and dumps every
candidate MADE crossing pair actually evaluated (not just the best one) for the
shots of interest. Makes no production code change.
"""
...
OUT_DIR = Path("data/diagnostics/arcvision_demo_final_forensic")
