...
overwritten or discarded), but it isn't surfaced per-shot in this featured session's UI. Showing
it disagree with the verified result (e.g. a verified MADE shot the classifier called MISSED)
reads as confusing rather than useful in a portfolio demo (see "Evaluation & Limitations" below
for how that classifier performs on video it wasn't tuned on). A normal local upload still shows
the automatic prediction directly, since there's no separate verified result to reconcile it
against.
...
- **No pretrained hoop/rim model**: none exists in COCO, and pulling an unverified
  ...
  candidate source, prompted directly as "basketball hoop/rim/backboard/net", with no color
  assumption, and confirmed working on that video. The same model, prompted as "basketball", was
...
- **Vanilla JS frontend**: this machine has no Node.js/npm installed, so a build-tool-free
...
overall and essentially never near the rim, which is exactly where make/miss detection needs
evidence most. It was replaced as the **primary** ball/rim source by a basketball-specific detector:
...
- **Runs 100% locally**: the fine-tuned checkpoint (`models/rfdetr_ball_rim_v1.pth`) is loaded
  from disk, with no network calls at inference time. Training required a one-time Roboflow API
  key (stored in a gitignored `.env`, never logged) to fetch the training data; nothing about
...
and this one was dark, weathered metal against a bright sky. It also showed that COCO's generic
...
benchmarked on one camera/lighting setup, so re-run `scripts/benchmark_ball_rim_detectors.py`
...
deflection or airball for a make) on the three videos already validated. That's a trade, not
a free fix, and judging whether a change actually helps would need a fifth video. Consistent with
